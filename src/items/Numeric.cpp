...
inline int compare(Numeric::State state1, const MAPM &value1, Numeric::State state2, const MAPM &value2)
{
  ...
  case Numeric::NEG_NUM:
    // std::cerr << "compare: " << value1.compare(value2) << "\n";
...
{
  // std::cerr << "this: " << UTF8(Numeric::asDecimalString(asMAPM(), 18, context))
  //           << ", other: " << UTF8(Numeric::asDecimalString(other->asMAPM(), 18, context))
  //           << "\n";
                                           ...
                                           const XMLCh* targetType, const DynamicContext* context) const
{
  ...
  case FLOAT:
    ...
    case NUM:
      if(targetType == 0) {
        return context->getItemFactory()->createFloat(asFloat(), context);
      }
      return context->getItemFactory()->createFloatOrDerived(targetURI, targetType, asFloat(), context);
    }
  case DOUBLE:
    switch(getState()) {
    case NaN:
    ...
    case NUM:
      if(targetType == 0) {
        return context->getItemFactory()->createDouble(asDouble(), context);
      }
      return context->getItemFactory()->createDoubleOrDerived(targetURI, targetType, asDouble(), context);
    }
  case BOOLEAN:
    if(targetType == 0) {
                      ...
                      const LocationInfo *info)
{
  ...
  double result = ::atan2(((Numeric*)y.get())->asDouble(),
    ((Numeric*)x.get())->asDouble());
                      ...
                      const LocationInfo *info)
{
  ...
  return (Item::Ptr)context->getItemFactory()->createDouble(::log10(((Numeric*)item.get())->asDouble()), context);
}
                      ...
                      const LocationInfo *info)
{
  Item::Ptr item = args[0]->createResult(context)->next(context);
  if(item.isNull()) return 0;
  return (Item::Ptr)context->getItemFactory()->createDouble(::exp10(((Numeric*)item.get())->asDouble()), context);
}
