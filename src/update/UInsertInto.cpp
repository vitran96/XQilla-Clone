...
UInsertInto::UInsertInto(ASTNode *source, ASTNode *target, XPath2MemoryManager* memMgr)
  : ASTNodeImpl(UINSERT_INTO, memMgr),
    source_(source),
    target_(target)
{
}
...
ASTNode *UInsertInto::staticTypingImpl(StaticContext *context)
{
  _src.clear();
...
Result UInsertInto::createResult(DynamicContext* context, int flags) const
{
  return 0;
}
  ...
  while((item = value->next(context)).notNull()) {
    if(((Node*)item.get())->dmNodeKind() == Node::attribute_string) {
      ...
      if(!UInsertAsLast::checkNamespaceBinding(qname, node, context, this)) {
        XMLBuffer buf;
        buf.append(X("Implied namespace binding for the insert into expression (\""));
